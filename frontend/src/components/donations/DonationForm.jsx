...
const DonationForm = ({ disasterId, organizations, onSuccess }) => {
    ...
    const fetchOrgNames = async () => {
      ...
      for (const orgAddress of organizations) {
        try {
          ...
          const orgData = await contract.getOrganization(addressToUse);
          names[addressToUse] = orgData[0] || orgData.name || addressToUse;
        } catch (err) {
          const addressToUse = typeof orgAddress === 'string' ? orgAddress : orgAddress.address;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Log off-chain donation to backend MongoDB
      try {
        const apiBase = import.meta.env.VITE_API_URL || 'http://localhost:5000';
        await fetch(`${apiBase}/api/donations`, {
          method: 'POST',
          headers: {
        ...
        console.warn("Failed to log donation off-chain:", dbErr);
      }
